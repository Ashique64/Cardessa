...
export default function CreateTemplate() {
  ...
  const [formData, setFormData] = useState({
    ...
    sort_order: 0,
    category_ids: [],
    style_tags_str: "Minimalist, Elegant",
    field_schema_str: JSON.stringify({
      fields: [
        { key: "groom_name", type: "text", label: "Groom's Name", required: true },
        { key: "bride_name", type: "text", label: "Bride's Name", required: true },
        { key: "event_date", type: "date", label: "Event Date", required: true },
        { key: "venue_name", type: "text", label: "Venue Name", required: true }
      ]
    }, null, 2),
    demo_content_str: JSON.stringify({
      groom_name: "Vikram",
      bride_name: "Ananya",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiClient.post("/templates/admin/", {
        name: formData.name,
        slug: formData.slug,
        tier: formData.tier,
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-brand-bg flex items-center justify-center font-sans">
        ...
        <span className="text-sm text-brand-text-muted font-medium">Verifying Credentials…</span>
      </div>
    );
  }
      ...
      <main className="max-w-xl w-full mx-auto px-6 pt-36 pb-24 grow">
        ...
        <header className="mb-10 space-y-1">
          ...
          <h1 className="font-serif text-3xl font-light text-brand-dark tracking-tight">
            Create <span className="italic font-normal">Template</span>
          </h1>
        </header>
